...
public class Persistencia {
    ...
    private static final String JPQL_FUNCIONARIO_POR_MATRICULA =
            "SELECT f FROM FuncionarioSistema f WHERE f.matricula = ?1";
    private static final String JPQL_FUNCIONARIO_POR_NOME =
            "SELECT f FROM FuncionarioSistema f " +
            "WHERE UPPER(f.nome) LIKE ?1";
    private static final String JPQL_TODOS_AMBIENTES =
            "SELECT a FROM Ambiente a ORDER BY a.descricao";
    ...
    public FuncionarioSistema findFuncionarioPorMatricula(String matricula)
            throws PersistenceException {
        if (matricula == null) {
            throw new NullPointerException("matricula não pode ser null");
        }
        EntityManager em = null;
        try {
            em = emf.createEntityManager();
            return (FuncionarioSistema) em
                    .createQuery(JPQL_FUNCIONARIO_POR_MATRICULA)
    ...
    public List<FuncionarioSistema> findFuncionariosPorNome(String nome)
            throws PersistenceException {
    	 if (nome == null) {
         ...
         try {
             em = emf.createEntityManager();
             return em.createQuery(JPQL_FUNCIONARIO_POR_NOME)
    ...
    //FIXME Gambiarra para usar sem o cartão 
	...
	public Permissao recuperarPermisao(FuncionarioSistema funcionario, Ambiente ambiente){		
		...
		("select p from Permissao p where (p.funcionario = :funcionario AND p.ambiente = :ambiente )");		
	...
	public void atualizarSenhaFuncionario(FuncionarioSistema funcionario){
        ...
        try {
        	 em = emf.createEntityManager();
            em.getTransaction().begin();	            
           Query query = em.createNativeQuery("UPDATE Funcionario SET senha ='"+funcionario.getSenha()
	...
	public boolean verificarPermissaoAmbiente(Ambiente ambienteSelecionado, FuncionarioSistema funcionario) {
		...
		try{
			em = emf.createEntityManager();
			Query query = 
			em.createNativeQuery("Select *From permissao " +
					"where (id_ambiente='"+ambienteSelecionado.getId()+"' " +
						"AND matricula_funcionario='"+funcionario.getMatricula()+"' )");
	...
	public FuncionarioSistema findFuncionarioPorNome(String nome) throws PersistenceException  {
		...
		em = emf.createEntityManager();		
		Query query = em.createQuery
		("select f from FuncionarioSistema f where (f.nome = :nome )");
	...
	public List<Permissao> recuperarPermicoesPorFuncionario(FuncionarioSistema funcionario){
		...
		em = emf.createEntityManager();		
		Query query = em.createQuery
		("select p from Permissao p where (p.funcionario = :funcionario )");
		query.setParameter("funcionario", funcionario);
	...
	//FIXME recuperarAmbientesNaoPermitidos
	...
	public void excluirPermissao(Permissao permissao) {
		...
		("delete from Permissao where(idpermissao='"+permissao.getIdPermissao()+"')");
		query.executeUpdate();
		em.getTransaction().commit();
		em.close();
	}
	...
	public void atualizarFuncionarioAtivo(FuncionarioSistema funcionario) {
        ...
        try {
        	 em = emf.createEntityManager();
            em.getTransaction().begin();	            
           Query query = em.createNativeQuery("UPDATE Funcionario SET ativo ='"+funcionario.isAtivo()
        		   +"' WHERE(matricula ='"+funcionario.getMatricula()+"')");
           query.executeUpdate();
          em.getTransaction().commit();          
